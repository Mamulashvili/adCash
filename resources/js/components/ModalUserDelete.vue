<template>
  <div v-if="showUser" class="wrapper flex flex-center flex-column">
    <p class="title fw-bold block mb-2">Delete user?</p>

    <p class="desc fs-default text-center fw-100 txt-primary">
      Are you sure to delete
      <span class="fw-bold txt-accent">'@{{ showUser.username }}'</span>?
      <br />
    ...
    <div class="warning-content flex brd-radius">
      ...
      <div class="ml-2">
        ...
        <p class="warning-desc">
          Maybe this user already purchased stocks. <br />
        </p>
      </div>
    </div>
...
export default {
  ...
  methods: {
    deleteUser: function () {
      HTTP.delete(`/users/${this.itemId}`)
        .then(({ status }) => {
          if (status === 200) this.$emit("closeModal");
        })
        .catch((error) => {
          console.log(error);
